...
void setup() {
  ...
  server.on("/", []() {
    ...
    // --- SIMPLE WIFI MANAGER SECTION ---
    html += "<div class='data-card' style='text-align:left'>";
    html += "<div class='label' style='margin-bottom:1rem'>Kelola Koneksi WiFi</div>";
    html += "<input type='text' id='ssid' placeholder='Nama WiFi (SSID)' style='width:100%;padding:0.8rem;margin-bottom:0.5rem;border-radius:0.8rem;border:none;background:rgba(255,255,255,0.1);color:white;box-sizing:border-box'>";
    html += "<input type='password' id='pass' placeholder='Password WiFi' style='width:100%;padding:0.8rem;margin-bottom:1rem;border-radius:0.8rem;border:none;background:rgba(255,255,255,0.1);color:white;box-sizing:border-box'>";
    html += "<button onclick='saveWiFi()' class='btn' style='background:#10b981;margin:0'>Update Koneksi</button>";
    html += "</div>";
    // ------------------------------------

    html += "<a href='/update' class='btn' style='background:#64748b;margin-top:1rem'>Update Firmware</a></div>";
    
    html += "<script>function saveWiFi(){";
    html += "const s=document.getElementById('ssid').value; const p=document.getElementById('pass').value;";
    html += "if(!s){alert('Isi Nama WiFi!');return;}";
    html += "fetch(`/api/wifi?s=${encodeURIComponent(s)}&p=${encodeURIComponent(p)}`).then(r=>r.text()).then(t=>alert(t));";
    html += "}";
    html += "setInterval(()=>{fetch('/api/data').then(r=>r.json()).then(d=>{";
  ...
  // API untuk Simpan WiFi (Easy Mode)
  server.on("/api/wifi", []() {
    if (!server.authenticate("admin", "NodeKRWBAR@01")) return server.send(401);
    String s = server.arg("s");
    String p = server.arg("p");
    if (s.length() > 0) {
      server.send(200, "text/plain", "Mencoba menyambungkan ke " + s + "... Alat akan restart otomatis jika berhasil.");
      delay(2000);
      WiFi.begin(s.c_str(), p.c_str());
      // WiFiManager akan otomatis menyimpan kredensial baru ini jika koneksi berhasil
    } else {
